...
@section('titleName', 'Danh sách các loại giường ngủ')
				...
				<div class="col-md-12">
					...
					<div class="portlet box blue-hoki">
						<div class="portlet-title">
							<div class="caption">
								<a href="{{ route('bed_types.create') }}" class="btn btn-default btn-sm">
						...
						<div class="portlet-body">
							...
							@foreach( $bed_types as $k => $s )
	                            <tr>
									...
									<td>
										<a href="{{ route('bed_types.edit', $s->id) }}" class="btn btn-sm btn-primary edit">
											<i class="fa fa-pencil"></i> Edit
										</a>
										<form style="display: -webkit-inline-box;" action="{{ route('bed_types.destroy', $s->id) }}" id="delete_form" method="POST">
											{{ csrf_field() }}
											{{ method_field('DELETE') }}
											<button onclick="return confirm('Bạn muốn xóa kiểu giường này?')" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i> Delete</button>
										</form>
									</td>
	                            </tr>
	                        @endforeach 
